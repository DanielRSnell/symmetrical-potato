...
class Mobile extends Component {
  ...
  CreateLink(props) {
    ...
    const Link = props.name.toLowerCase().replace(/ /g, "-").replace(/\./g, "-");
  ...
  CreateEnd(props) {
    ...
    if ( Check !== true ) {
      return (<span className="mobile-from">
        <strong>E: </strong>{m(props.end).format('l')}
      </span>);
    } else if ( Check !== false ) {
      return (
        <span className="mobile-from">
         <strong>E: </strong>{m(props.preend).format('l')}
        </span>
      )
    } else {
      return ( <span className="mobile-from">
        <strong>E: </strong><Tag>Unknown</Tag>
      </span>
      )
    }
  }
  ...
  CreateStart(props) {
    ...
    if ( Check !== true ) {
      return (<span className="mobile-from">
        <strong>S: </strong>{m(props.start).format('l')}
      </span>);
    } else if ( Check !== false ) {
      return (
        <span className="mobile-from">
         <strong>S: </strong>{m(props.prestart).format('l')}
        </span>
      )
    } else {
      return ( <span className="mobile-from">
        <strong>S: </strong><Tag>Unknown</Tag>
      </span>
      )
    }
  }
  ...
  CreateTag(props) {
    ...
    if ( CheckEnd === true ) {
      ...
      return (
        <Tag color="green">LIVE-ICO</Tag>
      )
    } else if ( CheckPre === false) {
      return (
        <Tag color="blue">PRE-ICO</Tag>
      )
    } else {
  ...
  CreateRating(props) {
    if ( props.rating !== null ) {
      if ( props.rating > 3.9 ) {
        ...
        return (
          <Tag color="green">{props.rating}</Tag>
        )
        ...
        <Row className="item-row" key={item.node.id}>
          <Row className="mobile-item-row" span={24}>
            ...
            <span className="mobile-class-item">
              <Avatar size="large" src={item.node.logo} className="mobile-avatar" />
            </span>
              <span className="mobile-class-item">
